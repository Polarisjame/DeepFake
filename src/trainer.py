...
from src.utils import Logger, AverageMeter, Drawer, plt, GpuInfoTracker, Monitor
...
class Trainer():
    ...
    def run_batch(self, feature, label, gpu_log=None):
        ...
        gpu_log(f"Classify Loss:{loss.item()}")
        # loss = loss_class + self.align_loss_rate*loss_diff
    ...
    def submit(self,dataset:DeepFakeSet):
        result_dict = {}
        logger = self.logger
